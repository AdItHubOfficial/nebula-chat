...
export default function AdminBadgesModal({ onClose }: { onClose: () => void }) {
  ...
  async function toggle(user: PublicUser, field: 'verified' | 'og') {
    const next = !user[field];
    setResults((rs) => rs.map((u) => (u.id === user.id ? { ...u, [field]: next } : u)));
    const badges = field === 'verified' ? { verified: next } : { og: next };
    try {
      const { user: updated } = await api.admin.setBadges(user.id, badges);
      setResults((rs) => rs.map((u) => (u.id === updated.id ? updated : u)));
      // keep our own badges fresh if we edited ourselves
      const me = useAuthStore.getState().user;
      if (me && updated.id === me.id) useAuthStore.getState().patchUser({ verified: updated.verified, og: updated.og });
  ...
  return (
    <Modal onClose={onClose} title="Founder Panel" subtitle="Search any account and grant badges." className="max-w-lg">
      ...
      <div className="min-h-[120px] space-y-1.5">
        ...
        {results.map((u) => (
          <div key={u.id} className="flex items-center gap-3 rounded-xl border border-line/50 bg-surface-2/50 p-2.5">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-1.5">
                ...
                <RoleBadges verified={u.verified} og={u.og} size={13} />
              </div>
              <span className="text-xs text-muted">@{u.username}</span>
            </div>
            <BadgeToggle active={u.verified} label="Verified" onClick={() => toggle(u, 'verified')}>
              <BadgeCheck size={13} /> Verified
            </BadgeToggle>
            <BadgeToggle active={u.og} label="OG" og onClick={() => toggle(u, 'og')}>
              OG
            </BadgeToggle>
          </div>
        ))}
...
function BadgeToggle({ active, onClick, children, og }: { active: boolean; onClick: () => void; children: React.ReactNode; label: string; og?: boolean }) {
  return (
    <button
      onClick={onClick}
      className={cn(
        'flex shrink-0 items-center gap-1 rounded-lg px-2.5 py-1.5 text-xs font-bold transition',
        active ? 'text-white' : 'bg-surface-3/70 text-muted hover:text-content',
      )}
      style={active ? (og ? { background: 'linear-gradient(135deg, rgb(var(--c-accent)), rgb(var(--c-accent-2)))' } : { background: '#3b9dff' }) : undefined}
    >
      {children}
    </button>
  );
